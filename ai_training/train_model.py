...
def _count_images(root: Path) -> int:
    """Count supported image files under the dataset root."""
    return sum(1 for path in root.rglob('*') if path.suffix.lower() in {'.png', '.jpg', '.jpeg', '.bmp'})


def ensure_demo_dataset(dataset_root: str) -> None:
    """Autogenerate the bundled demo dataset when the repo has no images checked in."""
    root = Path(dataset_root)
    if _count_images(root) > 0:
        return

    from dataset.generate_demo_dataset import generate_dataset

    print('No dataset images found. Generating the bundled synthetic demo dataset...')
    generate_dataset()
...
def train(dataset_root: str, output_dir: str = 'ai_training/output', epochs: int = 10) -> Path:
    ...
    ensure_demo_dataset(dataset_root)
