...
## Bundled Ready-to-Run Demo Dataset

To make the repository usable right away without committing binary assets, `dataset/` now includes a tiny synthetic demo dataset generator. The training workflow auto-generates demo images into the expected class folders if they are empty. The generator is stored at `dataset/generate_demo_dataset.py`, and the dataset notes are in `dataset/README.md`. This demo data is only for software validation and must not be treated as medical data.


### Dataset Layout

Store images like this (the repo auto-generates a tiny synthetic demo dataset into this exact layout when needed):
...
1. **Create a Python environment**
   ...
   ```
2. **Use the bundled auto-generated demo dataset immediately** or replace it with your own images under `dataset/train/...` and `dataset/val/...`.
3. **Optionally preprocess** the dataset:
   ```bash
   python preprocessing/preprocess.py --input dataset/train --output processed/train
   python preprocessing/preprocess.py --input dataset/val --output processed/val
   ```
4. **Train the model** (if `dataset/` is empty, the synthetic demo dataset will be generated automatically first)
   ```bash
   python ai_training/train_model.py --dataset_root dataset --epochs 10
   ```
5. **Convert to TFLite**
